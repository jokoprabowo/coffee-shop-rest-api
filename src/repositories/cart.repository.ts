...
class CartRepository {
  ...
  public async getCartItems(userId: number) {
    const query = {
      text: 'select ci.id as cart_item_id, co.name, co.price, '+
      'ci.quantity, (co.price * ci.quantity) as total_price_per_item '+
      'from carts c inner join cart_items ci on ci.cart_id = c.id '+
      'inner join coffees co on co.id = ci.coffee_id '+
      'where c.user_id = $1 order by ci.id',
      values: [userId],
    };
  ...
  public async updateItem(cartItemId: number, quantity: number) {
    const query = {
      text: 'update cart_items set quantity = $1 where id = $2',
