...
describe('Get monthly order stats endpoint', () => {
  let userId: number;
  let adminId: number;
  ...
  it('Should return a 200 status code and list of monthly order stats', async () => {
    const response = await request(app).get('/api/v1/orders/stats?month=2&year=2026&statuses=paid')
      .set('Authorization', `Bearer ${adminToken}`);

    expect(response.statusCode).toBe(200);
    expect(response.body.data).toHaveProperty('orderStats');
    expect(Array.isArray(response.body.data.orderStats)).toBe(true);
  });
