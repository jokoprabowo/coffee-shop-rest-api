...
describe('Get order details endpoint.', () => {
  ...
  it('Should return a 400 status code if the order id is not a number.', async () => {
    const response = await request(app).get('/api/v1/orders/abc')
      .set('Authorization', `Bearer ${userToken}`);
  ...
  it('Should return a 404 status code if the cart with provided id is not exist.', async () => {
    const response = await request(app).get(`/api/v1/orders/${orderId + 1}`)
      .set('Authorization', `Bearer ${userToken}`);
